...
export function FocusPeakingOverlay({
  ...
  useEffect(() => {
    ...
    if (cacheRef.current?.url === imageUrl) {
      ...
      const ctx = canvas.getContext('2d');
      if (ctx) {
        // Scale cached (possibly downsampled) overlay to full canvas
    ...
    const compute = async (): Promise<void> => {
      ...
      const edgeData = computeSobelEdges(imageData.data, w, h);

      if (cancelled) return;

      cacheRef.current = { url: imageUrl, data: edgeData };

      canvas.width = imageDimensions.width;
      canvas.height = imageDimensions.height;
      const ctx = canvas.getContext('2d');
      if (ctx) {
        // Draw downsampled result scaled up to full image dimensions
        const tmp = document.createElement('canvas');
        tmp.width = w;
        tmp.height = h;
        const tmpCtx = tmp.getContext('2d');
